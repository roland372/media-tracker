...
const NotesList = () => {
	...
	const handleDeleteNote = async id => {
		const filteredNotes = notesDatabase?.[0]?.notes?.filter(
			note => note.id !== id
		);

		notesDatabase[0].notes = filteredNotes;
	...
	//* sorting
	// const notesCopy = [...notesDatabase?.notes];
	const sortByDate = notesDatabase?.[0]?.notes?.sort(
		...
		<CardComponent title='Notes'>
			...
			{loading ? (
				...
				<div className='row px-2'>
					...
					{notesDatabase?.[0]?.notes ? renderNotes() : <Loader />}
